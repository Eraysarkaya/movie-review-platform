...
class Genre(models.Model):
    title = models.CharField(max_length=25)  # Türün başlığı maksimum 25 karakter uzunluğu
    slug = models.SlugField(null=False, unique=True) # Slug alanı, boş olamaz ve benzersiz olmalıdır

    def get_absolute_url(self):
        return reverse('genres', args=[self.slug]) # Türün detay sayfasına giden URL yi döner.
...
# Filmle ilgili detayların özellikleri karakter uzunluğu ve boş olup olmaması gibi
class Movie(models.Model):
    ...
    # Filmin posterini indirip, posteri isimlendirip, dosyaya kaydetip, dosyadan posteri kullanma
     def save(self, *args, **kwargs):
          if self.Poster == '' and self.Poster_url != '':
               resp = requests.get(self.Poster_url, timeout=10)
...
# İnceleme yazısı ilgili bilgiler
...
class Likes(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='user_like') # Beğeniyi yapan kullanıcı
    type_like = models.PositiveSmallIntegerField() # Beğeni tipi like/unlike
    review = models.ForeignKey(Review, on_delete=models.CASCADE, related_name='review_like')# Beğenilen yorum
